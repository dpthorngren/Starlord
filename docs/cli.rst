...
.. code:: none
    ...
    Grid mist2
      ...
      0 log_teff                  3.176      5.699         41     g.mist2Tracks.log_teff--i
      1 log_g                        -1        9.5         22     g.mist2Tracks.log_g--i
      2 feh                          -3        0.5         15     p.feh
      3 afe                        -0.2        0.6          5     p.afe
      4 Av                            0          6         13     p.Av
    Outputs
        Output                      Min        Max
      5 bc_Bessell_B             -18.82     0.9105
      6 bc_Bessell_I             -14.95      1.307
      7 bc_Bessell_R             -16.18      2.446
      8 bc_Bessell_U             -24.26     0.2244
      9 bc_Bessell_V             -17.41      2.529
    [...]
    600 VISTA_Y              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_VISTA_Y--i - 5*(math.log10(g ...
    601 VISTA_Z              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_VISTA_Z--i - 5*(math.log10(g ...
    602 Washington_C         -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_Washington_C--i - 5*(math.lo ...
    603 Washington_M         -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_Washington_M--i - 5*(math.lo ...
    604 Washington_T1        -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_Washington_T1--i - 5*(math.l ...
    605 Washington_T2        -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_Washington_T2--i - 5*(math.l ...
    606 WISE_W1              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_WISE_W1--i - 5*(math.log10(g ...
    607 WISE_W2              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_WISE_W2--i - 5*(math.log10(g ...
    608 WISE_W3              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_WISE_W3--i - 5*(math.log10(g ...
    609 WISE_W4              -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_WISE_W4--i - 5*(math.log10(g ...
    610 WIYN_NESSI_NB832     -2.5 * g.mist2Tracks.log_lum--i + 9.74 - g.mist2.bc_WIYN_NESSI_NB832--i - 5*(mat ...

In this case, the truncation [...] is in the documentation only; the actual output lists all 610 inputs, outputs and derived values.  For the inputs and outputs, the actual minimum and maximum values are shown so you can be sure your priors and likelihood terms fall within those bounds.  Inputs also list the length of the grid in that dimension and the default mapping; in this case we can see that the metallicity and extinction are set as parameters, whereas the effective temperature and gravity are calculated from the ``mistTracks`` grid.

.. note::

    The above instructions examine grids stored by starlord.  If you prefer to store them elsewhere, you can still get the grid summary with ``starlord path/to/mygrid.npz`` -- note that ``-g`` is now optional.
